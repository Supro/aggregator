...
//			direction:			"ltr",
			...
			allButtons: {
                ...
                html: '<div id="imguploader"><div class="progress" style="display: none;"><div class="progress-bar"></div></div><input type="file" name="image" id="add-image" multiple class="btn-new primary">Выбрать изображение</button><div class="error"></div></div>'
              }
						],
	...
	$.wysibb.prototype = {
		...
		imgLoadModal: function() {
			...
			if (this.options.imgupload===true) {
					var _this = this;
					this.$modal.find('#add-image').fileupload({
						url: '/api/v1/images',
						dataType: 'json',
						formData: {
							image_type: 'other'
							...
							_this.$txtArea.insertImage(data.result.url);

							_this.closeModal();
							_this.updateUI();

							setTimeout(function(){
								_this.$modal.find('.progress').css({'opacity': '0', 'display': 'none'});
								_this.$modal.find('.progress .progress-bar').css({'width': '0%'});
							}, 1000);
						},
						start: function () {
							_this.$modal.find('.error').css({'display': 'none'});
							_this.$modal.find('.progress').css({'opacity': '1', 'display': 'block'});
						},
						progressall: function(e, data){
							_this.$modal.find('.progress .progress-bar');
							_this.$modal.find('.progress .progress-bar').css({'width': parseInt(data.loaded / data.total * 100, 10) + '%'});
						},
						fail: function(e, data){
							_this.$modal.find('.error').css({'display': 'block'}).text(data.jqXHR.responseJSON.errors.file[0]);
							_this.$modal.find('.progress').css({'opacity': '0', 'display': 'none'});
							_this.$modal.find('.progress .progress-bar').css({'width': '0%'});
						}
					});
